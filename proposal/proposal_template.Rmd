...
```{r prelimdata, warning=FALSE, message=FALSE, echo=FALSE}
joineddata %>%
    filter(Entity %in% c("Saudi Arabia", "Germany", "Canada", "Spain", "United States")) %>%
    ggplot(mapping = aes (x = Year, y = `Deaths - Ambient ozone pollution - Sex: Both - Age: Age-standardized (Rate)`, width = 3, height = 3)) +
    theme(axis.text.x = element_text(angle=70, hjust=1)) +
    geom_line() +
    facet_wrap(~Entity, ncol = 3) +
    labs (x = "Months",
    y = "Count of Births",
    title = "Average Ozone Deaths")
```


```{r, echo=FALSE}
joineddata %>%
  filter(Entity %in% c("India", "China", "North Korea")) %>%
    ggplot(mapping = aes (x = Year, y = `Deaths - Ambient ozone pollution - Sex: Both - Age: Age-standardized (Rate)`, width = 3, height = 3)) +
    theme(axis.text.x = element_text(angle=70, hjust=1)) +
    geom_line() +
    facet_wrap(~Entity, ncol = 3) +
    labs (x = "Months",
    y = "Count of Births",
    title = "Monthly Birth Averages (2016-2021)")
```
