...
library(tidyverse)
library(readr)
death_rates_total_air_pollution <- read_csv("~/R/Project Proposal/data/death-rates-total-air-pollution.csv")
death_rates_from_air_pollution <- read_csv("~/R/Project Proposal/data/death-rates-from-air-pollution.csv")
...
For our plan, we are looking at several different predictors, mostly focusing on different types of air pollution (household, ambient matter, and ozone) and the outcomes we are looking at deaths. We will compare the different types of air pollution among each other and to other risk factors/predictors. Down below we created a four graphs that examine the trends of ozone related deaths in four different countries. In general there seems to be a downward trend with deaths associated with household air pollution, which makes sense as household air pollution has been decreasing due to alternative clean fuel options being introduced. We would like to see if that trend remains true in other countries and among other types of air pollution.
...
joineddata %>%
  filter(Entity %in% c("India", "China", "North Korea", "Nepal")) %>%
    ...
    labs (x = "Years",
    y = "Count of Deaths",
    title = "Yearly Death Averages due to Ambient Ozone Pollution (1990-2017)")
```
